...
func (d *DefaultSchema) CreateTable() []string {
	...
	for _, table := range tables {
		createTables = append(createTables, "CREATE TABLE "+table+" ("+
			d.ID()+" TEXT NOT NULL PRIMARY KEY,"+
			d.Payload()+" BLOB NOT NULL,"+
			d.SequenceNumber()+" INTEGER,"+
			d.ActorName()+" TEXT NOT NULL,"+
			d.Created()+" TEXT NOT NULL DEFAULT (DATETIME('now', 'localtime')),"+
			"UNIQUE ("+d.ID()+"),"+
